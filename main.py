#Set starting screen dimensions, this has to be set before any code
...
class MainWidget(RelativeLayout):
    ...
    V_NB_LINES = 8
    V_LINES_SPACING = 0.4 #percentage for line spacing in screen width
    vertical_lines = []
    H_NB_LINES = 15
    H_LINES_SPACING = 0.1 #percentage for line spacing in screen height
    horizontal_lines = []
    current_offset_y = 0
    current_offset_x = 0
    current_y_loop = 0
    ...
    def __init__(self, **kwargs):
        ...
        self.init_tiles()
        #ship needs to be created after the tiles so that ship spawns on top of the tiles
        self.init_ship()
        self.reset_game()
        #Create keyboard functionality if device is a desktop
        if self.is_desktop():
            #Binds keyboard to game
            self._keyboard = Window.request_keyboard(self.keyboard_closed, self)
            self._keyboard.bind(on_key_down=self.on_keyboard_down)
            self._keyboard.bind(on_key_up=self.on_keyboard_up)
        #Call self.update 60 times in one second
    ...
    def pre_fill_tiles_coordinates(self):
        #10 tiles in a straight line at the start
        for i in range(10):
    ...
    def update_vertical_lines(self):
        ...
        for i in range(start_index, self.V_NB_LINES+start_index):
            #floating numbers will make lines more visible than others
    ...
    #Makes vertical and horizontal lines "regenerate"
    def update(self, dt):
        time_factor = dt * 60
